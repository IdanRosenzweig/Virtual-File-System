...
node_id_t ram_driver::allocte_node() {
    node_id_t id = nodes_id_gen.generate_id();
    nodes_pool[id] = node(id);
    return id;
}

void ram_driver::deallocate_node(node_id_t id) {
...
bool ram_driver::has_node(node_id_t id) {
    return nodes_pool.count(id);
}

std::unique_ptr<node> ram_driver::read_node(node_id_t id) {
...
void ram_driver::write_node(const node *node) {
...
comp_id_t ram_driver::allocate_comp() {
    comp_id_t id = comps_id_gen.generate_id();
    comps_pool[id] = comp(null_comp(id));
    return id;
}

void ram_driver::deallocate_comp(comp_id_t id) {
...
bool ram_driver::has_comp(comp_id_t id) {
    return comps_pool.count(id);
}

comp_type ram_driver::read_comp_type(comp_id_t id) {
    return comp::get_type(comps_pool[id]);
}

std::unique_ptr<comp> ram_driver::read_comp(comp_id_t id) {
...
void ram_driver::write_comp(const comp *comp) {
...
refs_id_t ram_driver::allocte_refs() {
    refs_id_t id = refs_id_gen.generate_id();
    refs_pool[id] = refs(id);
    return id;
}

void ram_driver::deallocate_refs(refs_id_t id) {
...
bool ram_driver::has_refs(refs_id_t id) {
    return refs_pool.count(id);
}

std::unique_ptr<refs>  ram_driver::read_refs(refs_id_t id) {
...
void ram_driver::write_refs(const refs *refs) {
...
content_id_t ram_driver::allocate_content() {
    content_id_t id = content_id_gen.generate_id();
    content_pool[id] = content(null_content(id));
    return id;
}

void ram_driver::deallocate_content(content_id_t id) {
...
bool ram_driver::has_content(content_id_t id) {
    return content_pool.count(id);
}

content_type ram_driver::read_content_type(content_id_t id) {
    return content::get_type(content_pool[id]);
}

std::unique_ptr<content> ram_driver::read_content(content_id_t id) {
...
void ram_driver::write_content(const content *data) {
