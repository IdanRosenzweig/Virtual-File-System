...
protected:
    ...
    static ctx_t<node_id_t> mk_direct_node(ctx_t<node_id_t> ctx, const path_comp &p, const comp_data &comp_val,
    ...
    static ctx_t<std::unique_ptr<T>> read_comp(ctx_t<comp_id_t> ctx) {
        auto comp = read_comp(ctx);
        return {ctx.hier, std::make_unique<T>(*comp::get_ptr<T>(comp.val.get()))};
    }

    template <typename T>
static ctx_t<std::unique_ptr<T>> read_content(ctx_t<content_id_t> ctx) {
        auto comp = read_content(ctx);
...
public:
    ...
    /** retrieve the information of compponents and content */
    static ctx_t<std::unique_ptr<comp> > read_comp(ctx_t<comp_id_t> ctx) noexcept; // read comp

    static ctx_t<std::unique_ptr<refs> > read_refs(ctx_t<refs_id_t> ctx) noexcept; // read comp

    static ctx_t<std::unique_ptr<content> > read_content(ctx_t<content_id_t> ctx) noexcept; // read content
    ...
    void rm_mount(ctx_t<node_id_t> ctx, const path& p) noexcept;

    static ctx_t<node_id_t> mv_mount(ctx_t<node_id_t> ctx, const path& src, const path& dest) noexcept;
